...
local function main()
  ...
  if not tr then
    reaper.ShowMessageBox("Selecciona el track de narración.", "setgap_sentence", 0)
    return
  end
  ...
  if not pe or not ns then
    reaper.ShowMessageBox("Pon el cursor en el silencio entre dos items.", "setgap_sentence", 0)
    return
  end

  local gap = ns - pe
  if gap < -EPS then
    reaper.ShowMessageBox("No hay gap válido (items traslapados).", "setgap_sentence", 0)
    return
  end
  ...
  reaper.Undo_EndBlock(string.format("setgap_sentence: %.3fs", tgt), -1)
end
