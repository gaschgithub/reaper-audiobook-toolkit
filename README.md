...
- **`auto_pickup.lua`**
  Reading all error markers per pickup session, the script finds each error within the book script, highlights each error's sentence and exports all PDF pages with pickup annotations, as well as .mp3 pickup reference files (calls Python from Lua for PDF management).
