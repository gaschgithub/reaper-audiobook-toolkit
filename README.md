...
⚠️ **Work in Progress**  
This repository documents an evolving set of REAPER/Lua scripts developed in professional audiobook editing practice. The codebase is **not frozen** and may change as editing strategies evolve.
...
This repository does **not yet** attempt to:
...
- **`auto_pickup.lua`**
  Reading all error markers per pickup session, the script finds each error within the book script, highlights each error's sentence and exports all PDF pages with pickup annotations, as well as .mp3 pickup reference files <sub>(calls Python from Lua for PDF management)<sub>.
